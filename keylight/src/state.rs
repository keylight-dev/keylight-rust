...
pub enum LicenseState { Trial { days_left: i64 }, Licensed, Limited, FreeTier, Expired, Invalid }

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum TrialStatus { NotStarted, Active { days_left: i64 }, Expired }

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum KeylessState { Trial, FreeTier, Expired }
impl KeylessState {
    pub fn wire(&self) -> &'static str { match self { Self::Trial => "trial", Self::FreeTier => "free_tier", Self::Expired => "expired" } }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LicenseLifecycleEvent { Renewed, Cancelled, Expired, Restored }
...
pub fn resolve_state(
    lease_status: Option<&str>, lease_current: bool, had_license: bool,
    trial: &TrialStatus, free_tier_enabled: bool,
) -> LicenseState {
    ...
    if had_license { return LicenseState::Expired; }
    match trial {
        TrialStatus::Active { days_left } => LicenseState::Trial { days_left: *days_left },
...
mod tests {
    ...
    fn active_current_lease_is_licensed() {
        assert_eq!(resolve_state(Some("active"), true, true, &TrialStatus::NotStarted, false), LicenseState::Licensed);
    }
    #[test]
    fn fallback_is_limited() {
        assert_eq!(resolve_state(Some("fallback"), true, true, &TrialStatus::NotStarted, false), LicenseState::Limited);
    }
    #[test]
    fn no_license_trial_active_is_trial() {
        assert_eq!(resolve_state(None, false, false, &TrialStatus::Active { days_left: 5 }, false), LicenseState::Trial { days_left: 5 });
    }
    #[test]
    fn no_license_free_tier_is_free_tier() {
        assert_eq!(resolve_state(None, false, false, &TrialStatus::NotStarted, true), LicenseState::FreeTier);
    }
    #[test]
    fn nothing_is_invalid() {
        assert_eq!(resolve_state(None, false, false, &TrialStatus::NotStarted, false), LicenseState::Invalid);
    }
    #[test]
