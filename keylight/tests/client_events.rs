...
impl Transport for ValidInvalid {
    fn post_json(&self,_:&str,_:&[(String,String)],_:&str)->TransportOutcome { TransportOutcome::Response(HttpResponse{status:200,body:r#"{"valid":false}"#.into(),retry_after:None}) }
...
fn validate_runs_with_event_handler_registered() {
    ...
    let kl = Keylight::with_parts(cfg, store, Arc::new(ValidInvalid))
        .with_event_handler(move |_ev| { c2.fetch_add(1, Ordering::SeqCst); });
