...
impl Transport for Noop {
    fn post_json(&self, _: &str, _: &[(String, String)], _: &str) -> TransportOutcome {
        TransportOutcome::Response(HttpResponse { status: 200, body: "{}".into(), retry_after: None })
    }
...
fn deactivate_clears_last_state() {
    ...
    let store = Arc::new(EncryptedFileStore::at_dir(d, &FixedDeviceIdentity("dev".into())).unwrap());
    let cfg = KeylightConfig::builder("t", "p").build();
    let store_dyn: Arc<dyn keylight::LicenseStore> = Arc::clone(&store) as Arc<dyn keylight::LicenseStore>;
    ...
    assert_eq!(store.get_string(account::LAST_STATE).as_deref(), Some("Active"));
    ...
    let _ = kl.deactivate();
    assert!(store.get_string(account::LAST_STATE).is_none(), "LAST_STATE must be cleared by deactivate()");
}
