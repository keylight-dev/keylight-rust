...
impl Transport for MockOk {
    fn post_json(&self, _u: &str, _h: &[(String,String)], _b: &str) -> TransportOutcome {
        TransportOutcome::Response(HttpResponse { status: 200, body: self.0.clone(), retry_after: None })
    }
...
fn activate_rejects_unverifiable_lease() {
    ...
    let store = Arc::new(EncryptedFileStore::at_dir(dir, &FixedDeviceIdentity("d".into())).unwrap());
...
fn activate_invalid_format_returns_error_not_request() {
    ...
    let _ = std::fs::remove_dir_all(&dir);
    let store = Arc::new(EncryptedFileStore::at_dir(dir, &FixedDeviceIdentity("d".into())).unwrap());
    let cfg = KeylightConfig::builder("t", "p").key_prefix("NOTES").build();
