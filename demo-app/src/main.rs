...
fn notes_path() -> std::path::PathBuf {
    std::env::temp_dir().join("keylight-notes.txt")
}
fn load() -> Vec<String> {
...
fn save(n: &[String]) {
    let _ = std::fs::write(notes_path(), n.join("\n"));
}
...
fn main() {
    ...
    match cli.cmd {
        Cmd::Activate { key } => {
            let r = k.activate(&key).expect("activate");
            println!(
                "{}",
                if r.activated {
                    "Pro unlocked!"
        ...
        Cmd::Add { text } => {
            ...
            if !pro && n.len() >= 3 {
                eprintln!("Free tier is limited to 3 notes. Activate a Pro key to add more.");
                std::process::exit(1);
            }
            n.push(text);
            save(&n);
            println!("Added ({} notes).", n.len());
        }
        Cmd::List => {
            for (i, note) in load().iter().enumerate() {
                println!("{}. {note}", i + 1);
            }
        }
        Cmd::Export { path } => {
            if !k.has_entitlement("pro") {
                eprintln!("Export is a Pro feature. Activate a Pro key.");
                std::process::exit(1);
            }
            let mut f = std::fs::File::create(&path).expect("create");
            f.write_all(load().join("\n").as_bytes()).expect("write");
